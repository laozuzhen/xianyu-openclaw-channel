...
# OpenClaw Bridge API 路由
try:
    from bridge_api import bridge_router
    BRIDGE_ROUTER_AVAILABLE = True
except ImportError:
    logger.warning("⚠️ bridge_api 未找到，OpenClaw Bridge 功能不可用")
    BRIDGE_ROUTER_AVAILABLE = False
...
# 注册 OpenClaw Bridge API 路由
if BRIDGE_ROUTER_AVAILABLE:
    app.include_router(bridge_router)
    logger.info("✅ 已注册 OpenClaw Bridge API 路由: /api/bridge")
else:
    logger.warning("⚠️ OpenClaw Bridge API 路由未注册")
